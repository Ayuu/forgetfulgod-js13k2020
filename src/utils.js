...
const setCache = v => localStorage.setItem(CACHE, JSON.stringify(v || { level }))
const levelFromCache = () => {
  const cache = JSON.parse(localStorage.getItem(CACHE) || "{}")
  if (!cache.level || cache.level >= maps.length) {
    cache.level = 0
    setCache(cache)
  }
  return cache.level
...
const addDom = (innerHTML, className, onclick, parent, id) => {
  ...
  id && (d.id = id)
  className && d.classList.add(className)
  d.onclick = onclick
